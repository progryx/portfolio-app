...
export function firstNameRequired(value: string) {
  if (!isTruthy(value)) {
...
export function messageRequired(value: string) {
  if (!isTruthy(value)) {
    return ERRORS.emailRequired;
  }
...
export function validateContactForm({ email, firstName, message }: ContactFormFields) {
