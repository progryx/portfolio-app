...
export const mainReducer = (state = initialState, action: ActionTypes) => {
  switch (action.type) {
    ...
    case 'SELECT_USER': {
      const items = state.table.items.map((user) =>
        user.id === action.payload.id ? { ...user, checked: !user.checked } : user
      );
    ...
    case 'SELECT_ALL_USERS': {
      return {
        ...state,
        table: {
          items: state.table.items.map((user) =>
            action.payload.selected ? { ...user, checked: true } : { ...user, checked: false }
          ),
